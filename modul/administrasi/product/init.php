...
function the_products() {
    ...
    $app->post('/products/add/submit', function(Request $request)  {
        ...
        $db->bind("ref", $ref);
        // Execute
        $row = $db->query("INSERT INTO product(product_name,value,devident_rate,max_pairing,referral_rate) VALUES(:pname,:value,:devrate,:maxpair,:ref)");
        if ($row) {
    ...
    $app->post('/products/edit/submit', function(Request $request)  {
        ...
        $db->bind("prod_id", $id);
        // EXECUTE
        $row = $db->query("UPDATE product SET product_name = :pname,value = :value ,devident_rate = :dev_rate,max_pairing = :maxpair,referral_rate = :ref WHERE product_id = :prod_id");
        if ($row) {
            return new Response('SUCCESS', 200);
        } else {
            return new Response('Failed', 201);
