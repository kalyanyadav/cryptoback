...
function getSponserId($uid){
    global $db;
    $db->bind("uid",$uid);
    $downline = $db->query("SELECT sponsorid FROM genealogy WHERE uid=:uid");
    return $downline[0]['sponsorid'];
}
function getLevelBonus($pid){
    global $db;
    $db->bind("pid",$pid);
    $product = $db->query("SELECT level_bonus FROM product WHERE product_id=:pid");
    return $product[0]['level_bonus'];
}
